...
const coursesSearchHint = document.getElementById("coursesSearchHint");
...
function showCourses() {
    ...
    showSection("courses");
    const searchResult = filterCourses(coursesSearch.value);
    renderCourseList(searchResult.courses);
    updateCourseSearchHint(searchResult);
}
...
function filterCourses(query) {
    ...
    if (!cleaned) {
        return {courses, query: cleaned, usedFuzzy: false};
    }

    const scoredMatches = courses
        .map(course => ({course, score: scoreCourseMatch(course, cleaned)}))
        .filter(item => item.score > 0)
        .sort((left, right) => right.score - left.score || left.course.name.localeCompare(right.course.name));

    const exactMatchCount = scoredMatches.filter(item => isExactCourseMatch(item.course, cleaned)).length;

    return {
        courses: scoredMatches.map(item => item.course),
        query: cleaned,
        usedFuzzy: scoredMatches.length > 0 && exactMatchCount === 0,
    };
}

function updateCourseSearchHint(searchResult) {
    if (!coursesSearchHint) {
        return;
    }

    if (!searchResult.query || !searchResult.usedFuzzy) {
        coursesSearchHint.textContent = "";
        coursesSearchHint.classList.add("hidden");
        return;
    }

    coursesSearchHint.textContent = `Showing close matches for "${searchResult.query}".`;
    coursesSearchHint.classList.remove("hidden");
}
